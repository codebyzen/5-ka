...
if (!defined('PKASASS') || PKASASS!=md5('PKASASS')) { echo 'WTF?'; exit(); }
	...
	if (isset($_POST['title']) && isset($_POST['text']) && !empty($_POST['title']) && !empty($_POST['text'])) {
		file_put_contents("../data/articles/".$_POST['time'],$_POST['title']."\r\n".$_POST['text']);
		echo "<script>document.location.href='".$_cfg['www']."/admin/?act=listA'</script>";
		exit();
	}

	$_tmp=explode("\r\n",file_get_contents("../data/articles/".$_GET['article']));
	$_title=$_tmp[0];
	array_shift($_tmp);
	$_text=implode("\r\n",$_tmp);
	$_text=stripslashes($_text);
?>
<FORM METHOD="POST" id="addAForm" enctype="application/x-www-form-urlencoded" ACTION="<?=$_cfg['www'];?>/admin/?act=editA">
<input type="text" name="title" value="<?=$_title;?>">
<input type="hidden" name="time" value="<?=$_GET['article'];?>">
...
<input type="button" value="Ok" onClick="document.getElementById('addAForm').submit();" style="width:563px;">